...
In the preceding example, we set up the following three virtual VIEWS:

* `StockPrice` gets the stock price from the `quotessvc`
* `AccountValues` view uses the `StockPrice` view to determine the cumulative account value for all of the customers in the `AccountsDB` database.
* `CustomerHoldings` stores the value of each individual customer's account based on the stock shares that they own. 


This value proposition of the Data Virtualization is the end user sees only the `AccountValue` and `StockPrice` view.
They do not know how this data is integrated together.
...
Name: portfolio
Driver: Teiid
URL: jdbc:teiid:portfolio@mm://127.0.0.1:31000
User: <user>
Password: <password>
----
